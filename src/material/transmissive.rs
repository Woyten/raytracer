...
impl Material for Transmissive {
    fn get_color(
        ...
        let refraction = if normal_projection > 0.0 {
            self.refraction.recip()
        } else {
            self.refraction
        };
        let tangential_space = direction - normal_projection * normalized_normal;
        let discriminant = refraction * refraction * normal_projection * normal_projection
            + (refraction * refraction - 1.0) * tangential_space.norm_squared();
        ...
        if is_total_reflection {
            ...
            let refracted_ray = Ray {
                start: reflection_point,
                direction: tangential_space
                    + normal_projection.signum() * discriminant.sqrt() * normalized_normal,
            };
            let transmitted_color = self
                .color
                .component_mul(&refracted_ray.trace(scene, num_recursions - 1));
